$(function () {
    ...
    function colorelemento(elemento,opc) {
        ...
        if(nota > 0 && nota < 40){
            ...
            else if($("div[name=" + id + "]").find('p').hasClass("azul")){
    ...
    function contabilizar(elemento) {
        var id=$(elemento).closest("div.grid-col");
        var destino=$(elemento).closest("div.grid-col").attr('name');
        if (elemento != null) {
            for (var k = 0; k < 3; k++) {
                ...
                switch (k) {
                    ...
                    case 0:
                        var total_1 = $(id).find('.estadoguia_u option:selected[value="1"]').length;
                        var total_2 = $(id).find('.estadoguia_u option:selected[value="2"]').length;
                        var total_3 = $(id).find('.estadoguia_u option:selected[value="3"]').length;
                        $("div[name=10_" + destino + "]").replaceWith('<div class="grid-item-pond-header" style="background-color: orange;height: 75px;width: 219px;"  name="10_'+destino+'"><p style="color:white;">Entregada a Tiempo: ' + total_1 + '</p><p style="color:white;">Entregada a Destiempo: ' + total_2 + '</p><p style="color:white;">No Entregada: ' + total_3 + '</p></div>');
                        break;
                    case 1:
                        var total_4 = $(id).find('.estadoguia_d option:selected[value="1"]').length;
                        var total_5 = $(id).find('.estadoguia_d option:selected[value="2"]').length;
                        $("div[name=30_" + destino + "]").replaceWith('<div class="grid-item-pond-header" style="background-color: green;height: 75px;width: 219px;" name="30_'+destino+'"><p style="color:white;">Entregada Completa: ' + total_4 + '</p><p style="color:white;">Entregada Incompleta: ' + total_5 + '</p></div>');
                        break;
                    case 2:
                        var total_6 = $(id).find('.estadoguia_t option:selected[value="1"]').length;
                        var total_7 = $(id).find('.estadoguia_t option:selected[value="2"]').length;
                        var total_8 = $(id).find('.estadoguia_t option:selected[value="3"]').length;
                        $("div[name=60_" + destino + "]").replaceWith('<div class="grid-item-pond-header" style="background-color: cyan;height: 75px;width: 219px;"  name="60_'+destino+'"><p>Logro entre 80% y 100%: ' + total_6 + '</p><p>Logro entre 60% y 79%: ' + total_7 + '</p><p>Logro menor al 60%: ' + total_8 + '</p></div>');
                        break;
                    ...
                    if (data.notas == '' || data.notas == 'Null') {
                        ...
                        for (var i = 0; i < data.notas.length; i++) {
                            ...
                            for (var k = 0; k < largo; k++) {
                                ...
                                switch (data.notas[i]['alumnos'][k]['valorGuia']) {
                                    ...
                                    case 1:
                                        ...
                                        }else if(data.notas[i]['alumnos'][k]['ponderacion_10']==0){
                            ...
                            for (var k = 0; k < largo; k++) {
                                ...
                                if(data.notas[i]['alumnos'][k]['nota']==0){

                                    $("div#porcentaje" + data.notas[i]['idEvaluacion']).append('<div class="grid-item selector" tabindex="e' + inde + '" name="' + data.notas[i]['alumnos'][k]['idNotas'] + '" style="text-align: center;display: block;"><p>-</p></div>');
                            ...
                            for (var k = 0; k < 1; k++) {
                                ...
                                switch (k) {
                                    ...
                                    case 0:
                                        ...
                                        $("div#guia" + data.notas[i]['idEvaluacion']).append('<div style="display: -webkit-inline-flex; width: 676px"><div class="grid-item-pond-header" style="background-color: orange;height: 75px;width: 219px;" tabindex="es' + ind + '" name="10_'+data.notas[i]['idEvaluacion']+'"><p style="color:white;">Entregada a Tiempo: ' + total_1 + '</p><p style="color:white;">Entregada a Destiempo: ' + total_2 + '</p><p style="color:white;">No Entregada: ' + total_3 + '</p></div><div class="grid-item-pond-header" style="background-color: green;height: 75px;width: 219px;" tabindex="es' + ind + '" name="30_'+data.notas[i]['idEvaluacion']+'"><p style="color:white;">Entregada Completa: ' + total_4 + '</p><p style="color:white;">Entregada Incompleta: ' + total_5 + '</p></div><div class="grid-item-pond-header" style="background-color: cyan;height: 75px;width: 219px;" tabindex="es' + ind + '"  name="60_'+data.notas[i]['idEvaluacion']+'"><p>Logro entre 80% y 100%: ' + total_6 + '</p><p>Logro entre 60% y 79%: ' + total_7 + '</p><p>Logro menor al 60%: ' + total_8 + '</p></div></div>');
                                        break;
                                ...
                                $("div#porcentaje" + data.notas[i]['idEvaluacion']).append('<div class="grid-item" style="height: 80px;" tabindex="es' + ind + '"><p></p></div>');
    ...
    $('select').live('change', function (e) {
        ...
        if (clase == 'estadoguia_u') {
            ...
            if (id == 3) {
                ...
                ($(elemento).find(".estadoguia_d option:first").val()==0)?$(elemento).find(".estadoguia_d option:first").val(0):$(elemento).find(".estadoguia_d").prepend(new Option("-", "0"));
                ($(elemento).find(".estadoguia_n_d option:first").val()==0)?$(elemento).find(".estadoguia_n_d  option:first").val(0):$(elemento).find(".estadoguia_n_d ").prepend(new Option("-", "0"));
                ($(elemento).find(".estadoguia_t option:first").val()==0)?$(elemento).find(".estadoguia_t option:first").val(0):$(elemento).find(".estadoguia_t").prepend(new Option("-", "0"));
                ($(elemento).find(".estadoguia_n_t option:first").val()==0)?$(elemento).find(".estadoguia_n_t option:first").val(0):$(elemento).find(".estadoguia_n_t").prepend(new Option("-", "0"));


                //$(elemento).find(".estadoguia_u").val();
                $(elemento).find(".estadoguia_n_u").val(0);
                $(elemento).find(".estadoguia_d").val(0);
                $(elemento).find(".estadoguia_n_d").val(0);
                $(elemento).find(".estadoguia_t").val(0);
                $(elemento).find(".estadoguia_n_t").val(0);
                $(elemento).find(".estadoguia_n_u").attr('disabled', 'disabled');
                $(elemento).find(".estadoguia_d").attr('disabled', 'disabled');
                $(elemento).find(".estadoguia_n_d").attr('disabled', 'disabled');
                $(elemento).find(".estadoguia_t").attr('disabled', 'disabled');
                ...
                if(id==2){
                    //$(elemento).find(".estadoguia_d option:first").val()==0?$(elemento).find(".estadoguia_d option:first").remove();

                    if($(elemento).find(".estadoguia_d option:first").val()==0){
                        $(elemento).find(".estadoguia_d option:first").remove();
                    }
                    if($(elemento).find(".estadoguia_n_d option:first").val()==0){
                        $(elemento).find(".estadoguia_n_d option:first").remove();
                    }
                    if($(elemento).find(".estadoguia_t option:first").val()==0){
                        $(elemento).find(".estadoguia_t option:first").remove();
                    }
                    if($(elemento).find(".estadoguia_n_t option:first").val()==0){
                        $(elemento).find(".estadoguia_n_t option:first").remove();
                    }
                    $(elemento).find(".estadoguia_n_u").val(50);
                    if($(elemento).find(".estadoguia_n_d").val()==30){
